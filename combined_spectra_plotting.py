...
parser.add_argument("-dirg", "--directories_gaussian", default='None', metavar='DIRG', dest="directories_list_gaussian",
                     type=str, help="path of the directories containing the Gaussian .log file")
parser.add_argument("-diro", "--directories_orca", default='None', metavar='DIRO', dest="directories_list_orca",
                     type=str, help="path of the directories containing the Gaussian .log file")
parser.add_argument("-titg", "--title_gaussian", default='None', metavar='TITG', dest="title_list_gaussian",
                     type=str, help="path of the directories whose name has to be used to differentiate different gaussian tddft calculations (e.g. md001, md002,...")
parser.add_argument("-tito", "--title_orca", default='None', metavar='TITO', dest="title_list_orca",
                     type=str, help="path of the directories whose name has to be used to differentiate different orca tddft calculations (e.g. md001, md002,...")
...
log_files = glob.glob(args.directories_list_gaussian)
out_files = glob.glob(args.directories_list_orca)
titlesg = glob.glob(args.title_list_gaussian)
titleso = glob.glob(args.title_list_orca)
#######################################################################################################
...
#Individuate the block of Orca output that bears the information about the excited states.
#The first line of the block starts with "ABSORPTION SPECTRUM VIA TRANSITION ELECTRIC DIPOLE MOMENTS" and the first line following the block starts with "ABSORPTION SPECTRUM VIA TRANSITION VELOCITY DIPOLE MOMENTS"


for out_file in out_files:
    orca_output=open(out_file,'r')
    extracted=[]
    v=0
    for line in orca_output:
        splitted=line.split()
        if len(splitted)>5 and splitted[0]=='ABSORPTION' and splitted[1]=='SPECTRUM' and  splitted[2]=='VIA' and splitted[3]=='TRANSITION' and splitted[4]=='ELECTRIC':
            v=1
        if len(splitted)>5 and splitted[0]=='ABSORPTION' and splitted[1]=='SPECTRUM' and  splitted[2]=='VIA' and splitted[3]=='TRANSITION' and splitted[4]=='VELOCITY':
            v=0
        if v==1:
            extracted.append(splitted)
...
#f_i is a list of the oscillator strengths of the excitations found in the ith log file
    ...
    t=0
    for i in range(len(extracted)):
        if len(extracted[i])>2 and extracted[i][1]=='->':
            t=t+1
            excited_state_i.append(t)
            wavelength_i.append(float(extracted[i][5]))
            f_i.append(float(extracted[i][6]))
...
for i in range(len(titles)):
    ...
    for j in range(len(excited_state[i])):
        bars.write('\t\t')
        bars.write(f'{excited_state[i][j]:>3}{wavelength[i][j]:>8.2f}{f[i][j]:>8.4f}')
